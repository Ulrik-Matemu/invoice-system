...
const getPackageName = (): string => {
    const packageName = functions.config().app?.package_name;
    if (!packageName) {
        throw new functions.https.HttpsError(
            "failed-precondition",
            "Package name not configured. Set app.package_name in Firebase config."
        );
    }
    return packageName;
};
...
/**
 * Subscription payment states from Google Play:
 * 0 - Payment pending
 * 1 - Payment received
 * 2 - Free trial
 * 3 - Pending deferred upgrade/downgrade
 */
type PaymentState = 0 | 1 | 2 | 3;
...
export const verifySubscription = functions.https.onCall(async (data, context) => {
    ...
    if (!purchaseToken || typeof purchaseToken !== "string") {
        throw new functions.https.HttpsError(
            "invalid-argument",
            "A valid purchaseToken is required."
        );
    }
    ...
    if (!sku || typeof sku !== "string") {
        throw new functions.https.HttpsError(
            "invalid-argument",
            "A valid sku (subscription ID) is required."
        );
    }
    ...
    try {
        ...
        const paymentState = subscription.paymentState as PaymentState | undefined;
        const cancelReason = subscription.cancelReason;

        // 5. Check if subscription is in an Active state
        const { isActive, reason } = isSubscriptionActive(
            paymentState,
            expiryTimeMillis,
            cancelReason
        );
        ...
        if (error && typeof error === "object" && "code" in error) {
            ...
            if (apiError.code === 404) {
                throw new functions.https.HttpsError(
                    "not-found",
                    "Subscription not found. The purchase token may be invalid or expired."
                );
            }
            if (apiError.code === 401 || apiError.code === 403) {
                throw new functions.https.HttpsError(
                    "permission-denied",
                    "Server not authorized to verify subscriptions. Please contact support."
                );
            }
        }
        ...
        throw new functions.https.HttpsError(
            "internal",
            "Failed to verify subscription with Google Play. Please try again later."
        );
    }
});
